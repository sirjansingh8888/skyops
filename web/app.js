...
async function loadMetrics() {
  ...
  try {
    ...
    el.innerHTML = `<div class="card"><div class="card-title">Trajectory prediction <span class="muted">${esc(m.trajectory.predictor)}</span></div>
        <table><thead><tr><th>Horizon</th><th>Median</th><th>Mean</th><th>p90</th><th>n</th></tr></thead><tbody>${trajRows}</tbody></table><div class="hint">${esc(m.trajectory.note)}.</div>
        ${m.trajectory.gru ? `<div class="hint"><b>Learned alternative (not deployed):</b> a GRU that corrects dead reckoning, trained on ${m.trajectory.gru.samples} samples and tested on ${m.trajectory.gru.val_samples} samples from held-out aircraft, changed mean error by ${m.trajectory.gru.improvement_pct.map((p, i) => `${p > 0 ? '−' : '+'}${Math.abs(p)}% @${m.trajectory.gru.horizons[i]} s`).join(', ')}${m.trajectory.gru.turning && m.trajectory.gru.turning.n ? ` and gave no gain on the ${m.trajectory.gru.turning.n} turning-aircraft samples` : ''}. The physics baseline stays in production.</div>` : ''}</div>
      ...
      <div class="card"><div class="card-title">Drone perception</div><div class="muted small">${esc(p.model)}</div>${p.map50 !== undefined ? `<div class="kv"><div><span>mAP@50 (VisDrone val)</span>${fmt(p.map50, 3)}</div><div><span>mAP@50-95</span>${fmt(p.map50_95, 3)}</div><div><span>precision / recall</span>${fmt(p.precision, 2)} / ${fmt(p.recall, 2)}</div></div>` : '<div class="hint">Fine-tune on VisDrone to replace the placeholder (GPU run pending).</div>'}
        ${p.auair_transfer ? `<table><thead><tr><th>On AU-AIR demo frames (never trained on)</th><th>Recall</th><th>Precision</th></tr></thead><tbody>${Object.entries(p.auair_transfer).map(([k, v]) => `<tr><td>${esc(k)}</td><td><b>${fmt(v.recall * 100)}%</b></td><td>${fmt(v.precision * 100)}%</td></tr>`).join('')}</tbody></table><div class="hint">Class-agnostic match at IoU ≥ 0.3 over ${Object.values(p.auair_transfer)[0].frames} evenly spaced frames (${Object.values(p.auair_transfer)[0].gt_boxes} annotated objects).</div>` : ''}</div>
      <div class="card"><div class="card-title">Land use</div><div class="muted small">${esc(lu.model)}</div>${lu.best_miou !== undefined ? `<div class="kv"><div><span>mIoU (12 held-out tiles)</span>${fmt(lu.best_miou, 3)}</div><div><span>epochs</span>${lu.epochs}</div><div><span>best classes</span>${Object.entries(lu.per_class_iou || {}).filter(([, v]) => v === v && v !== null).sort((a, b) => b[1] - a[1]).slice(0, 2).map(([k, v]) => `${k} ${fmt(v, 2)}`).join(', ')}</div></div>` : `<div class="hint">Placeholder pixel accuracy ${lu.pixel_accuracy !== null ? fmt(lu.pixel_accuracy * 100) + '%' : '–'}; U-Net training pending (GPU).</div>`}</div>`;
